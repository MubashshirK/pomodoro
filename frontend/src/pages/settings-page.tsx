...
export default function SettingsPage() {
  ...
  // Reset the form when the backend settings arrive. The theme field
  // intentionally takes the provider's CURRENT value (not the saved
  // backend value), so the Select always matches whatever the topbar
  // is showing — the provider is the single source of truth for theme.
  useEffect(() => {
    if (data?.settings) {
  ...
  // Keep the form's theme in sync with the provider (e.g. user changes
  // theme from the topbar while on another page and then comes here).
  useEffect(() => {
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-sm text-muted-foreground">
          ...
          <p className="text-xs">
            CS50 Final Project · Vite + React + Flask
          </p>
        </CardContent>
      </Card>
    </div>
